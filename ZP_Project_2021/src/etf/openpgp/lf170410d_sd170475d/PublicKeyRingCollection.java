...
public class PublicKeyRingCollection {
	...
	public PGPPublicKey get(String user_id) {
		/*
		 * try { if(public_ring_collection.getKeyRings(user_id).hasNext()) {
		 * System.out.println("aaa"); PGPPublicKeyRing keyRing =
		 * public_ring_collection.getKeyRings(user_id).next(); Iterator<PGPPublicKey>
		 * kIt = (Iterator<PGPPublicKey>) keyRing.getPublicKeys(); PGPPublicKey key =
		 * null; while (key == null && kIt.hasNext()) { PGPPublicKey k = kIt.next();
		 * 
		 * if (k.isEncryptionKey()) { key = k; } } return key;
		 * 
		 * } } catch (PGPException e) { // TODO Auto-generated catch block
	...
	public PGPPublicKeyRing getRing(String user_id) {
		try {
			if (public_ring_collection.getKeyRings(user_id).hasNext()) {
				return public_ring_collection.getKeyRings(user_id).next();
			}
		} catch (PGPException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public void export_key_ring(String user_id, String filename) {
		PGPPublicKeyRing bc_public = getRing(user_id);
		if (bc_public != null) {
			try {
				BufferedOutputStream out = new BufferedOutputStream(
						new FileOutputStream("public_key_" + user_id + "_" + filename + ".pkr"));
	...
	public void import_key_ring(String filename) {
		try {
			...
			in.close();
			public_ring_collection = PGPPublicKeyRingCollection.addPublicKeyRing(public_ring_collection, bc_public);
	...
	public PGPPublicKey get(String user_id, long keyId) {
		try {
			if (public_ring_collection.getKeyRings(user_id).hasNext()) {
				java.util.Iterator<PGPPublicKeyRing> iter = public_ring_collection.getKeyRings(user_id);
				// asd
				PGPPublicKeyRing skr = iter.next();
				if (keyId == 0) {
				...
				while (skr != null) {
					if (skr.getPublicKey(keyId) != null) {
						Iterator<PGPPublicKey > it =skr.getPublicKeys();
						it.next();
						return it.next();
					}
	...
	public void remove(String user_id, long keyId) {
		try {
			if (public_ring_collection.getKeyRings(user_id).hasNext()) {
				...
				PGPPublicKeyRing skr = iter.next();
				if (keyId == 0) {
					public_ring_collection= PGPPublicKeyRingCollection.removePublicKeyRing(public_ring_collection, skr);
					return;
				}
	...
	public PGPPublicKey get(long key_id) {
    	try {
